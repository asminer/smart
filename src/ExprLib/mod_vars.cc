...
void model_var::SetNextState(traverse_data &x, shared_state* ns, long rhs) const
{
  DCASSERT(x.answer);
  x.answer->setNull();
}
...
{
  if (bounds) bounds->GetElement(i, foo);
  else        foo.setNull();
}

long model_statevar::GetValueIndex(result& foo) const
{
  if (bounds) return bounds->IndexOf(foo);
  else        return -1;
}
...
void model_statevar::AddToState(traverse_data &x, long delta) const
{
  ...
  if (bounds) {
    ...
    if (newst<0) boundsError(x, newst);
  }
...
void model_statevar
::SetNextState(traverse_data &x, shared_state* ns, long rhs) const
{
  DCASSERT(x.answer);
  DCASSERT(x.current_state);
...
void model_statevar::ownerError(traverse_data &x) const
{
  ...
  const hldsm* hm = x.current_state->Parent();
  DCASSERT(hm);
  DCASSERT(hm->GetParent() != getParent());
  if (hm->StartError(x.parent)) {
    em->cerr() << "state variable " << Name() << " belongs to another model";
    hm->DoneError();
  }
  DCASSERT(x.answer);
  x.answer->setNull();
...
int_statevar::int_statevar(const symbol* wrapper, const model_instance* p, shared_object* b)
 : model_statevar(wrapper, p, b)
{
  DCASSERT(wrapper);
  DCASSERT(wrapper->Type() == em->INT);
}
...
shared_state::shared_state(const hldsm* p)
{
  ...
  if (p->hasPartInfo()) {
    ...
    for (int i=0; i<=num_substates; i++) substate_offset[i] = 0;
  } else {
    substate_offset = 0;
    num_substates = 0;
  }
...
{
  DCASSERT(parent == s.parent);
  DCASSERT(isFixedSize() == s.isFixedSize());
  DCASSERT(getNumStateVars() == s.getNumStateVars());
  ...
  if (has_unknown) {
    if (!is_unknown) is_unknown = new bool[num_buckets];
...
class model_symbol : public symbol {
  symbol* link;
public:
  model_symbol(const char* fn, int line, const type* t, char* n);
...
model_symbol::model_symbol(const char* fn, int line, const type* t, char *n)
 : symbol(fn, line, t, n)
{
  link = 0;
...
{
  DCASSERT(x.which == traverse_data::Compute);
  DCASSERT(x.answer);
...
class model_array : public array {
  array* link;
public:
  model_array(const char* fn, int line, const type* t, char* n, iterator** il, int dim);
  virtual ~model_array();
  ...
  inline void ClearLink() { link = NULL; }

  virtual array_item* GetItem(expr** indices, result &x);
  virtual void Traverse(traverse_data &x);
};

model_array::model_array(const char* fn, int line, const type* t,
...
{
  if (link)  return link->GetItem(indices, x);
  x.setNull();
  return 0;
}
...
protected:
  ...
  int numvars;
public:
  model_varray_stmt(const char *fn, int line, model_def *p, const type* t, model_array** a, int nv);
  ...
  if (msr_slot < 0) {
    ...
    if (msr_slot < 0) {
      if (em->startInternal(__FILE__, __LINE__)) {
        em->causedBy(this);
        em->internal() << "Couldn't find slot for measure " << wrapper->Name();
        if (parent->Name()) em->internal() << " in model " << parent->Name();
        em->stopIO();
      }
      return;
    }
...
expr* clev_op::buildAnother(expr *r) const 
{
  ...
  DCASSERT(pl);
  return new clev_op(Filename(), Linenumber(), lower, pl);
}
...
public:
  blev_op(const char* fn, int line, expr* b, model_var* v);
  virtual void Compute(traverse_data &x);
  virtual void Traverse(traverse_data &x);
protected:
...
expr* blev_op::buildAnother(expr *l, expr *r) const 
{
  model_var* pl = smart_cast <model_var*>(r);
  DCASSERT(pl);
  return new blev_op(Filename(), Linenumber(), l, pl);
}
...
class vltc_op : public unary {
  long upper;
public:
  vltc_op(const char* fn, int line, model_var* v, expr* b);
  vltc_op(const char* fn, int line, model_var* v, long b);
  virtual void Compute(traverse_data &x);
  virtual void Traverse(traverse_data &x);
...
expr* vltc_op::buildAnother(expr *r) const 
{
  model_var* pl = smart_cast <model_var*>(r);
  DCASSERT(pl);
  return new vltc_op(Filename(), Linenumber(), pl, upper);
}
...
public:
  vltb_op(const char* fn, int line, model_var* v, expr* b);
  virtual void Compute(traverse_data &x);
  virtual void Traverse(traverse_data &x);
protected:
  virtual expr* buildAnother(expr *l, expr *r) const;
...
expr* vltb_op::buildAnother(expr *l, expr *r) const 
{
  ...
  DCASSERT(pl);
  return new vltb_op(Filename(), Linenumber(), pl, r);
}
...
public:
  ...
  clevltc_op(const char* fn, int line, long lb, model_var* v, long ub);
  virtual void Compute(traverse_data &x);
  virtual void Traverse(traverse_data &x);
  virtual bool Print(OutputStream &s, int w) const;
protected:
...
expr* clevltc_op::buildAnother(expr *r) const 
{
  model_var* pl = smart_cast <model_var*>(r);
  DCASSERT(pl);
  return new clevltc_op(Filename(), Linenumber(), lower, pl, upper);
}
...
void blevltb_op::Compute(traverse_data &x)
{
  ...
  // What cases are left?
  DCASSERT(l.isUnknown());
  DCASSERT(r.isUnknown());
  answer->setUnknown();
}
...
void vupdate_op::Traverse(traverse_data &x)
{
  switch (x.which) {
    ...
    case traverse_data::GetSymbols:
        var->Traverse(x);
        if (dec_amount) dec_amount->Traverse(x);
        if (inc_amount) inc_amount->Traverse(x);
        return;
...
class cassign_op : public expr {
  ...
  long rhs;
public:
  cassign_op(const char* fn, int line, model_var* v, long c);
  virtual ~cassign_op();
  virtual void Traverse(traverse_data &x);
  virtual void Compute(traverse_data &x);
  virtual bool Print(OutputStream &s, int) const;
...
class vassign_op : public expr {
  ...
  expr* rhs;
public:
  vassign_op(const char* fn, int line, model_var* v, expr* c);
  virtual ~vassign_op();
  virtual void Traverse(traverse_data &x);
  virtual void Compute(traverse_data &x);
  virtual bool Print(OutputStream &s, int) const;
        ...
        char* n, symbol** indexes, int dim) const
{
  ...
  return new model_array(fn, ln, t, n, (iterator**) indexes, dim);
}
    ...
    const type* t, expr* bnds, symbol** names, int N) const
{
  ...
  if (!bailout) if (bnds) {
    if (! isPromotable(bnds->Type(), t->getSetOfThis()) ) {
      ...
      bnds = makeTypecast(fn, ln, t->getSetOfThis(), bnds);
      DCASSERT(isOrdinary(bnds));
    }
  }
  // check the names
  if (!bailout) for (int i=0; i<N; i++) {
  ...
  return new model_var_stmt(fn, ln, p, t, bnds, (model_symbol**) names, N);
}
      ...
      const type* t, symbol** arrays, int N) const
{
  ...
  if (bailout) {
    for (int i=0; i<N; i++)  Delete(arrays[i]);
    delete[] arrays;
    return 0;
  }

  return new model_varray_stmt(fn, ln, p, t, (model_array**) arrays, N);
}
      ...
      model_def* p, symbol* am, expr* rhs) const
{
  ...
  rhs = promote(rhs, t);
  DCASSERT(! isError(rhs) );
  return new measure_array_assign(fn, ln, p, w, rhs);
}
...
expr* MakeBleVltB(const exprman* em, expr* lb, model_var* sv, expr* ub)
{
  ...
  if (0==lb) {
    if (ub_isconst) {
      answer = new vltc_op(0, -1, sv, ub);
    } else {
      answer = new vltb_op(0, -1, sv, ub);
    }
  } else if (0==ub) {
    if (lb_isconst)
      answer = new clev_op(0, -1, lb, sv);
    else
...
expr* MakeVarUpdate(const exprman* em, model_var* sv, expr* dec, expr* inc)
{
  ...
  if (dec_isconst && inc_isconst) {
    ...
    answer = new cupdate_op(0, -1, sv, inc_val - dec_val);
  } else {
    answer = new vupdate_op(0, -1, sv, dec, inc);
  }
...
expr* MakeVarAssign(const exprman* em, model_var* sv, expr* rhs)
{
  ...
  if (rhs->BuildExprList(traverse_data::GetSymbols, 0, 0)==0) {
    ...
    rhs->Compute(x);
    DCASSERT(foo.isNormal());
    answer = new cassign_op(0, -1, sv, foo.getInt());
  } else {
    answer = new vassign_op(0, -1, sv, rhs);
  }
...
{
  if (0==sv) return 0;
  expr* answer = new cassign_op(0, -1, sv, rhs);
#ifdef DEBUG_VASSIGN
  em->cout() << "Made vassign expr: ";
  answer->Print(em->cout(), 0);
  em->cout() << "\n";
