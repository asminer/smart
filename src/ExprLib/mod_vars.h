...
public:
	inline const model_instance* getParent() const {
		return parent;
	}

	inline storage getStateType() const {
		return state_type;
	}
...
public:
	...
	inline int GetPart() const {
		return part_index;
	}
	inline void SetPart(int p) {
		part_index = p;
	}
...
public:
	long value;
public:
	int_statevar(const symbol* wrapper, const model_instance* p,
			shared_object* bnds);
	virtual void Compute(traverse_data &x);
...
public:
	inline const hldsm* Parent() const {
		return parent;
	}
	inline const int* readState() const {
		return data;
	}
	inline int* writeState() {
		return data;
	}
	...
	inline int getNumStateVars() const {
		return num_buckets;
	}
