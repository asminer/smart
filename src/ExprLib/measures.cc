    ...
    : symbol(e->Filename(), e->Linenumber(), e->Type(), 0)
{
  ...
  if (model_debug.startReport())
  {
    ...
    if (rhs)
      rhs->Print(model_debug.report(), 0);
    else
      model_debug.report() << "null";
    model_debug.report() << "\n";
    model_debug.stopIO();
  }
  SetRHS(rhs);
...
void measure::notifyFrom(const symbol *p)
{
  ...
  if (class_deps)
  {
    ...
    {
      if (!class_deps->ReadItem(i)->isComputed())
        return;
    } // for i
  ...
  if (solve_deps)
  {
    ...
    {
      if (!solve_deps->ReadItem(i)->isComputed())
        return;
    } // for i
...
{
  if (which_engine != em->BLOCKED_ENGINE)
    return;
...
time_measure ::time_measure(const expr *e, model_def *p, expr *rhs)
...
void timeorder_msrs::addMeasure(measure *m)
{
  ...
  DCASSERT(tm);
  if (tm->GetTime() < current)
    next_round.Insert(tm);
  else
    this_round.Insert(tm);
}
...
msr_noengine ::msr_noengine(eng_class ec, const type *t, const char *name, int nf)
...
measure *msr_noengine ::buildMeasure(traverse_data &x, expr **pass, int np)
{
