...
indexed_statedbs::indexed_statedbs(StateLib::state_db &tdb,
		StateLib::state_db &vdb) :
		tandb(tdb), vandb(vdb) {
...
public:
	indexed_reachgraph(StateLib::state_db &tdb, StateLib::state_db &vdb,
			GraphLib::dynamic_digraph &rg);
	~indexed_reachgraph();
	// required
	inline static bool statesOnly() {
...
indexed_reachgraph::indexed_reachgraph(StateLib::state_db &tdb,
		StateLib::state_db &vdb, GraphLib::dynamic_digraph &therg) :
...
indexed_smp::indexed_smp(hldsm &m, StateLib::state_db &tdb,
		StateLib::state_db &vdb, LS_Options &vs,
		MCLib::vanishing_chain &the_smp) :
...
protected:
	 ...
	 */
	void generateRG(dsde_hlm* m, StateLib::state_db* rss, LS_Vector &s0,
			GraphLib::dynamic_digraph* rg) const;
	...
	/** Build the reachability set and (maybe) Markov chain.
	 ...
	 @throw  Appropriate error code.
	 */
	void generateMC(dsde_hlm* m, StateLib::state_db* ss, LS_Vector &s0,
...
void as_procgen::RunEngine(hldsm* hm, result &statesonly) {
	DCASSERT(hm); DCASSERT(AppliesToModelType(hm->Type()));
	...
	if (lm) {
		...
		if (statesonly.getBool())
			return;
		if (e != this)
			return e->RunEngine(hm, statesonly);
	}
	...
	if (nondeterm) {
		...
		if (slm) {
			...
			if (rg)
				the_proc = "reachability set & graph";
			else
				the_proc = "reachability set";
		}
	} else {
		if (slm) {
			rss = slm->getRSS()->getStateDatabase();
			...
			if (statesonly.getBool())
				...
				the_proc = "reachability set & Markov chain";
		} DCASSERT(rss);
		if (!statesonly.getBool()) {
...
// **************************************************************************
// *                                                                        *
// *                            as_procgen class   For Coverability         *
// *                                                                        *
// **************************************************************************

class as_procgenCOV: public process_generator {
	const exp_state_lib* statelib;
public:
	as_procgenCOV(const exp_state_lib* sl);
...
protected:
	...
	void generateRG(dsde_hlm* m, StateLib::state_db* rss, LS_Vector &s0,
			GraphLib::dynamic_digraph* rg) const;
//Coverability
	lchild_rsiblingt* generateCT(dsde_hlm* m, StateLib::state_db* rss,
			LS_Vector &s0, GraphLib::dynamic_digraph* rg) const;
...
as_procgenCOV::as_procgenCOV(const exp_state_lib* sl) :
		process_generator() {
	statelib = sl;
}

bool as_procgenCOV::AppliesToModelType(hldsm::model_type mt) const {
	return (hldsm::Asynch_Events == mt);
}
//TODO need cleanup
void as_procgenCOV::RunEngine(hldsm* hm, result &statesonly) {
	...
	StateLib::state_db* rss = 0;
	StateLib::state_db* rss1 = 0;

	GraphLib::dynamic_digraph* rg = 0;
	GraphLib::dynamic_digraph* cg = 0;
	MCLib::vanishing_chain* vc = 0;
	const char* the_proc = 0;
	...
	if (nondeterm) {
		if (!statesonly.getBool()) {
			rg = new GraphLib::dynamic_digraph(true);
			cg = new GraphLib::dynamic_digraph(true);

		}
		if (slm) {
			rss = slm->getRSS()->getStateDatabase();
			rss1 = slm->getRSS()->getStateDatabase();

			DCASSERT(rg);
			the_proc = "Coverability graph";
		} else {
			rss = statelib->createStateDB(true, false);
			rss1 = statelib->createStateDB(true, false);

			if (rg)
				the_proc = "reachability set & graph";
			if (cg)
				the_proc = "Coverability";
...
void as_procgenCOV::generateRG(dsde_hlm* dsm, StateLib::state_db* tandb,
		LS_Vector &s0, GraphLib::dynamic_digraph* rg) const {
	DCASSERT(dsm);DCASSERT(tandb);
...
lchild_rsiblingt* as_procgenCOV::generateCT(dsde_hlm* dsm,
		StateLib::state_db* tandb, LS_Vector &s0,
		GraphLib::dynamic_digraph* rg) const {
	DCASSERT(dsm);DCASSERT(tandb);
...
void as_procgenCOV::generateMC(dsde_hlm* dsm, StateLib::state_db* tandb,
		LS_Vector &s0, MCLib::vanishing_chain* smp) const {
	DCASSERT(dsm);DCASSERT(tandb);
