...
class exact_ph_avg : public exact_mcmsr {
public:
  exact_ph_avg();
  virtual bool AppliesToModelType(hldsm::model_type) const;
...
// **************************************************************************
// *                          exact_ph_avg methods                          *
// **************************************************************************

exact_ph_avg::exact_ph_avg() : exact_mcmsr()
{
}

bool exact_ph_avg::AppliesToModelType(hldsm::model_type mt) const
{
  return (hldsm::Phase_Type==mt);
}


subengine::error exact_ph_avg
::RunEngine(hldsm* foo, result &fls)
{
  ...
  if (proc->knownMTTA())  return Success;
  ...
  if (goal < 0) {
    ...
    proc->setTrapProb(1);
    proc->setInfiniteMTTA();
    proc->setVTTA(0); // constant infinity has 0 variance
    return Success;
  }
  long trap = proc->getTrapState();
  if (1==ns) {
  ...
  if (!proc->computeClassProbs(init, mx)) {
    doneTimer(watch);
    return Engine_Failed;
  }
  ...
  // Part of our job - save accept and trap probs.
  //
  proc->setAcceptProb(mx[goal]);
  if (trap >= 0) {
    proc->setTrapProb(mx[trap]);
  } else {
    proc->setTrapProb(0);
  }
...
class exact_ph_var : public exact_mcmsr {
public:
  exact_ph_var();
  virtual bool AppliesToModelType(hldsm::model_type) const;
  virtual void RunEngine(hldsm* m, result &parm);
...
// **************************************************************************
// *                          exact_ph_var methods                          *
// **************************************************************************

exact_ph_var::exact_ph_var() : exact_mcmsr()
{
}

bool exact_ph_var::AppliesToModelType(hldsm::model_type mt) const
{
  return (hldsm::Phase_Type==mt);
}
