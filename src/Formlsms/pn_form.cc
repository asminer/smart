...
public:
  ...
  // required for hldsm:
  virtual void showDec(OutputStream &s, const shared_state* x)const;
...
void petri_hlm::showDec(OutputStream &s, const shared_state* st) const
{
  DCASSERT(st);
...
class petri_def : public dsde_def {
  ...
  static const type* decision_type;
...
const type* petri_def::decision_type;
...
bool petri_formalism::canDeclareType(const type* vartype) const
{
  ...
  if (vartype->matches("decision"))  return 1;
  return 0;
}
...
pn_decisions::pn_decisions(const type* ds): model_internal(ds,"decisions",1) //: model_internal(ps, "decisions", 1)
{
  dset = ds;
  DCASSERT(dset);
  SetDocumentation("Returns the set of decisions of the Petri net.");
}

void pn_decisions::Compute(traverse_data &x, expr** pass, int ndd)
{
  DCASSERT(x.answer);
  DCASSERT(0==x.aggregate);
  //DCASSERT(1==ndd);
  ...
  int ndl = mypn->getNumDecVars();//write for decision
  
  if (0==ndl) {
    // empty set
    x.answer->setPtr(MakeSet(dset->getSetElemType(), 0, 0, 0));
    return;
  }
  
  // build a set of decisions
  result* dlist = new result[ndl];
  for (int i=0; i<ndl; i++) {
    dlist[i].setPtr(Share(mypn->getDecVar(i)));//decisions
  }
  x.answer->setPtr(MakeSet(dset->getSetElemType(), ndl, dlist));
...
bool init_pnform::execute()
{
  ...
  // set up and register decision types
  simple_type* t_decision  = new void_type("decision", "Petri net decision", "Decisions taken currently in Petri net, can alter the label.");
  t_decision->setPrintable();
 ...
 // Add_DSDE_varfuncs(petri_def::decision_type, pnsyms);
  Add_MCC_varfuncs(petri_def::place_type, pnsyms);
  //Add_MCC_varfuncs(petri_def::decision_type, pnsyms);
