...
//////decisions
void state_lldsm::showDecs(bool internal) const
{
  DCASSERT(RSS);// what will be this?
  if (internal) {
    //RSS->showInternal(em->cout());
  } else {
    long num_decs;
    //RSS->getNumDecs(num_decs);//which parameters
    ///if (tooManyStates(num_states, &(em->cout()))) return; //??????

//why would i pass num_decs as argument?
   // RSS->getNumDecs(num_decs);
    //shared_state* st = new shared_state(parent);
    //RSS->showDecs(em->cout(), stateDisplayOrder(), st);//
    //Delete(st);
  }
}
///////////
