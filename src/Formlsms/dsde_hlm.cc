...
const char* model_event::nameOf(firing_type t) {
	...
	case Hidden:
		return "Hidden (and non-deterministic)";
	case Immediate:
		return "Immediate";
	case Expo:
		return "Timed (exponential)";
	case Phase_int:
		return "Timed (phase int)";
	case Phase_real:
		return "Timed (phase real)";
	case Timed_general:
		return "Timed (general)";
	default:
		return "Unknown";
	}
...
void model_event::setTimed(expr *dist) {
	DCASSERT(Unknown == FT); DCASSERT(0==distro);
	if (0 == dist)
		return;
	distro = dist;
	distro->PreCompute();
...
void model_event::setWeight(int cl, expr *w) {
	DCASSERT(0==weight); DCASSERT(cl>0);
	wc = cl;
	weight = w;
	if (weight)
		weight->PreCompute();
...
void model_event::finishPriorityInfo(List<model_event> &keep,
		List<model_event> *ignored) {
	has_prio_level = true;
...
//	fprintf(stderr,"MAMAMAMAMAM%s\n",Name());
...
#endif
	DCASSERT(x.answer); DCASSERT(unknown == enable_data);
	for (int i = prio_length - 1; i >= 0; i--) {
		DCASSERT(prio_list[i]);
		if (prio_list[i]->unknownIfEnabled())
			prio_list[i]->decideEnabled(x);
	...
	if (x.answer->isNormal() && x.answer->getBool()) {
		enable_data = enabled;
#ifdef DEBUG_ENABLED
	fprintf(stderr, "SET TO ENABLED");
#endif
	} else {
		enable_data = disabled;
#ifdef DEBUG_ENABLED
	fprintf(stderr, "SET TO disable");
#endif
	}
#ifdef DEBUG_ENABLED
...
bool dsde_hlm::containsListVar() const {
	for (int i = 0; i < num_vars; i++) {
		DCASSERT(state_data);
		if (state_data[i]->getStateType() == model_var::Int_List)
			return true;
	}
	return false;
}
...
void dsde_hlm::checkAssertions(traverse_data &x) {
	...
	for (int i = 0; i < num_assertions; i++) {
		DCASSERT(assertions); DCASSERT(assertions[i]);
...
void dsde_hlm::checkVanishing(traverse_data &x) {
	...
	for (int pl = 0;; pl++) {
		...
		if (pl + 1 == num_priolevels)
			break;  // no more immediates to check
...
void dsde_hlm::makeEnabledList(traverse_data &x, List<model_event> *EL) {
	...
	for (int pl = 0; pl < num_priolevels; pl++) {
		for (; i < last_timed[i]; i++) {
			...
			if (x.answer->getBool() && EL)
				EL->Append(event_data[i]);
		} // for i
	} // for pl
}

void dsde_hlm::makeVanishingEnabledList(traverse_data &x,
		List<model_event> *EL) {
	DCASSERT(x.answer); DCASSERT(x.current_state);
	if (EL)
		EL->Clear();
...
void dsde_hlm::makeTangibleEnabledListCov(traverse_data &x,
	...
	int i = 0;
	bool has_enabled = false;
	printf("num_priolevels%d", num_priolevels);
	for (int pl = 0; pl < num_priolevels; pl++) {
		for (; i < last_immed[pl]; i++) {
			DCASSERT(event_data[i]);
			event_data[i]->setDisabled();
			...
			if (x.answer->getBool()) {
				...
				if (EL)
					if (!event_data[i]->omegaIfEnabled()) {
						EL->Append(event_data[i]);
						printf("IN makeTangibleEnabledList4 %s\n",event_data[i]->Name());
					}
...
void dsde_hlm::makeTangibleEnabledList(traverse_data &x,
		List<model_event> *EL) {
	DCASSERT(x.answer); DCASSERT(x.current_state);
	if (EL)
		EL->Clear();
...
void dsde_def::SetLevelOfStateVars(const expr* call, int level,
		shared_set* pset) {
	DCASSERT(pset);
	result elem;
	...
	for (int z = 0; z < pset->Size(); z++) {
		...
		DCASSERT(foo);
		if (!isVariableOurs(foo, call, "ignoring partition assignment"))
			continue;
...
void dsde_def::PartitionVars(model_statevar** V, int nv) {
	...
	} DCASSERT(min_g <= 0); DCASSERT(max_g >= 0);
...
void dsde_def::SetPriorityLevel(const expr* call, int level, shared_set* tset) {
	...
	for (int z = 0; z < tset->Size(); z++) {
		...
		model_var* foo = smart_cast <model_var*>(elem.getPtr());
		DCASSERT(foo);
		if (!isVariableOurs(foo, call, "ignoring priority assignment"))
			continue;
...
dsde_part1::dsde_part1(const type* place) :
	...
	SetRepeat(1);
	SetDocumentation(
			"Groups state variables into submodels.  Each call to this version of partition makes a new group of state variables.  Equivalent to calling partition({first set}:-1, {second set}:-2, ...)");
}

void dsde_part1::Compute(traverse_data &x, expr** pass, int np) {
	DCASSERT(x.answer); DCASSERT(0==x.aggregate); DCASSERT(pass); DCASSERT(pass[0]);
...
dsde_part2::dsde_part2(const type* place) :
	...
	SetRepeat(1);
	SetDocumentation(
			"Groups state variables into submodels.  All state variables in the set {vset} will be added to group <pnum>.  pnum must be positive, to prevent conflicts with the other forms of this function.");
...
dsde_part3::dsde_part3(const type* place) :
	...
	SetRepeat(1);
	SetDocumentation(
			"Groups state variables into submodels.  All state variables in the set {vset} will be added to the current group of state variable p.");
}
...
init_dsde::init_dsde() : initializer(__FILE__, 2)
{
    builds_resource("dsde_hlm.cc");
    needs_resource(dsde_def::dup_part.optName());
}
...
void init_dsde::execute()
{
    initialize_msg(dsde_def::dup_part,
        ...
        get_object(dsde_def::dup_part.optName())
    );
