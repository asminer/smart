...
class sos_promotions : public general_conv {
    public:
        ...
        virtual expr* convert(const char* file, int line, expr* e, const type* t) const {
            return new converter(file, line, t, e);
        }

    protected:
        class converter : public typecast {
            public:
                converter(const char* file, int line, const type* nt, expr* x);
...
sos_promotions::converter::converter(const char* file, int line, const type* nt,
        expr* x) : typecast(file, line, nt, x)
{
}
...
init_sos2sset::init_sos2sset() : initializer(__FILE__)
{
    buildsResource("sos2sset");
    usesResource("model_state");
    usesResource("statesettype");
}

bool init_sos2sset::execute()
{
    //
    // Register type conversion
    //
    em->registerConversion(new sos_promotions);
    return true;
}
