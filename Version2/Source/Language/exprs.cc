...
expr* symbol::Substitute(int i)
{
  ...
  if (substitute_value) {
    ...
    x.answer = &answer;
    Compute(x);
    return MakeConstExpr(Type(0), answer, Filename(), Linenumber());
  }
  // Don't substitute
  return Copy(this);
}
...
void aggregates::Compute(compute_data &x)
{
  DCASSERT(x.answer);
  int i = x.aggregate;
  x.aggregate = 0;
  CHECK_RANGE(0, i, opnd_count);
  if (operands[i]) operands[i]->Compute(x);
