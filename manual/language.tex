...
For example,
\begin{quote}
\Unix{smart ff1.sm ff2.sm - ff3.sm - ff4.sm}
\end{quote}
...
of verbosity.
Options statements appear on a single line beginning with ``\Code{\#}''
...
range, $[-2^{31},2^{31}-1]$ on most machines.
...
\PRIVATE{Need to discuss ``\Code{,}'' in formatting of \Code{bigint}}


\IGNORE{
\subsection{A \Code{real} iterator}

\PRIVATE{Is this something we want to forbid from future versions?
...
\PRIVATE{We might want to drop sets of reals...}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.75]{NestedDep.pdf}
...
\\
\RELEASE{Currently, {\smart} does not support Montecarlo simulation.}
...
\\
\RELEASE{Currently, {\smart} does not support Montecarlo simulation, thus no
operation can be performed on functions of type
\Code{rand int} or \Code{rand real} as of now.}
...
\begin{figure}  
  \centering
  \includegraphics[scale=0.75]{DTMC1.pdf}
...
The expression \Code{avg(pi\_tta)} evaluates to the average, or mean,
time-to-absorption (often called ``MTTA'').
Note that the function we use, \Code{pi\_tta}, has type \Code{ph int},
since it is the time-to-absorption of a DTMC;
had this been a CTMC model, we would have used \Code{pr\_tta}, with
type \Code{ph real}.
...
\begin{figure}
  \centering
  \includegraphics[scale=0.75]{DTMC3.pdf}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.75]{CTMC1.pdf}
...
\begin{lstlisting}
for (real lambda in {0.03..0.08..0.02}) {
  print("lambda=",lambda,"   reward=",c1(lambda).m1:10:8,"\n");
}
\end{lstlisting}
...
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\RELEASE{Currently, these functions must appear at the top level
of an expression defining a measure.
Thus,
\begin{code}
  \Code{real m1 := avg\_ss(cond(in\_state(a),2,cond(in\_state(b),1,0)));}
\end{code}
is correct, but the seemingly equivalent
\begin{code}
  \Code{real m1 := 2*prob\_ss(in\_state(a))+prob\_ss(in\_state(b));}
\end{code}
is rejected by {\smart}.
}
